...
Direct `workflow_dispatch` avoids depending on a tag pushed with `GITHUB_TOKEN` triggering another workflow. Legacy tag pushes still enter the same gates. Engram announces successful publication in Slack for both dispatch and tag-push entry points. Its preflight requires the configured webhook, and the notification step checks Slack's acknowledgment. A failed notification is reported separately from the already published release; do not rerun publication to resend it.
